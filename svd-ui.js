(function(){
  ...
  function compactTopbar(){
    ...
    if(eyebrow)eyebrow.textContent='DOC-CAPA';if(title)title.textContent='Gestión documental';
  }

  function buildUtilities(){
    const actions=document.querySelector('.topbar-actions');if(!actions||document.getElementById('svdUtilityActions'))return;
    const wrap=document.createElement('div');wrap.id='svdUtilityActions';wrap.className='svd-utility-actions';
    UTILITY_VIEWS.filter(x=>available(x.view)).forEach(item=>{const b=document.createElement('button');b.type='button';b.className='svd-utility-btn';b.dataset.svdUtility=item.view;b.textContent=item.label;b.addEventListener('click',()=>go(item.view));wrap.appendChild(b);});
    actions.appendChild(wrap);
  }

  function buildDocumentNavigation(){
    const top=document.querySelector('.topbar');if(!top||document.getElementById('svdNavShell'))return;
    ...
    SECTION_TABS.filter(x=>available(x.view)).forEach(item=>{const b=document.createElement('button');b.type='button';b.className='svd-section-tab';b.dataset.svdView=item.view;b.textContent=item.label;b.addEventListener('click',()=>go(item.view));tabs.appendChild(b);});
  }
  ...
  function bindNavigate(){
    ...
    const base=window.navigate;if(typeof base!=='function')return;
  ...
  function bindSavedState(id){
    ...
    btn.addEventListener('click',()=>{const wasDirty=core?.isDirty?.('document-layout');setTimeout(()=>{if(wasDirty&&!core?.isDirty?.('document-layout')){const original=btn.dataset.svdOriginalText||btn.textContent;btn.dataset.svdOriginalText=original;btn.textContent='Guardado';btn.classList.add('svd-saved');setTimeout(()=>{btn.textContent=original;btn.classList.remove('svd-saved');},1800);}},0);});
  }
  ...
  function directEntry(){
    ...
    if(!current||current==='inicio'){
      const first=SECTION_TABS.find(x=>available(x.view));if(first)go(first.view);
